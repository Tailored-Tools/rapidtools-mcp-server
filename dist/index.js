...
// ─── Tool: validate ────────────────────────────────────────────
server.tool("validate", "Validate a JSON payload against a JSON Schema. Returns a signed attestation proving validation occurred. Deterministic: same inputs always produce the same result.", {
    ...
    const { status, data } = await apiCall("/v1/validate", "POST", { schema, payload, metadata }, headers);
    if (status >= 400) {
        return {
...
// ─── Tool: verify ──────────────────────────────────────────────
server.tool("verify", "Verify the cryptographic signature of a validation attestation. Confirm that a previous validation result has not been tampered with.", {
    attestation_id: z
        .string()
        .optional()
...
server.tool("get_attestation", "Retrieve a stored validation attestation by its ID. Returns the full attestation record including schema hash, payload hash, validation result, and cryptographic signature.", {
    attestation_id: z.string().describe("The attestation ID to retrieve"),
}, async ({ attestation_id }) => {
    const { status, data } = await apiCall(`/v1/attestations/${encodeURIComponent(attestation_id)}`, "GET");
    if (status >= 400) {
        return {
            content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
            isError: true,
